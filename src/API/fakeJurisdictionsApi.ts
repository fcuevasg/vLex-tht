/**
 * Added support for TypeScript to be able to have intellisense and type check
 */

import { JurisdictionType } from './fakeJurisdictionType'

const SPAIN_JSON: JurisdictionType = {
	id: 1,
	name: 'Spain',
	subJurisdictions: [
		{
		...
		},
		{
			...
			subJurisdictions: [{ id: 10301, name: 'Asturias' }],
		},
		{
			id: 104,
			name: 'Balears, Illes',
			subJurisdictions: [{ id: 10401, name: 'Balears' }],
		},
		{
			id: 105,
			name: 'Canarias',
		...
		},
		{
			id: 106,
			name: 'Cantabria',
			subJurisdictions: [{ id: 10601, name: 'Cantabria' }],
		},
		{
			id: 107,
			name: 'Castilla y León',
			subJurisdictions: [
				{ id: 10701, name: 'Ávila', subJurisdictions: [{ id: 107011, name: 'Arenas de san pedro' }] },
		...
		},
		{
			...
			subJurisdictions: [{ id: 11301, name: 'Madrid' }],
		},
		{
			id: 114,
			name: 'Murcia, Región de',
			subJurisdictions: [{ id: 11401, name: 'Murcia' }],
		},
		{
			id: 115,
			name: 'Navarra, Comunidad Foral de',
			subJurisdictions: [{ id: 11501, name: 'Navarra' }],
		},
		{
			id: 116,
			name: 'País Vasco',
		...
		},
		{
			id: 117,
			name: 'Rioja, La',
			subJurisdictions: [{ id: 11701, name: 'La Rioja' }],
		},
		{
			id: 118,
			name: 'Ceuta',
			subJurisdictions: [{ id: 11801, name: 'Ceuta' }],
		},
		{
			id: 119,
			name: 'Melilla',
			subJurisdictions: [{ id: 11901, name: 'Melilla' }],
		},
	],
}

//Changed this to be just a regular array since with typing, the previous way was redundant
const fakeJurisdictionsData: JurisdictionType[] = [
	SPAIN_JSON,
	{
		id: 2,
		name: 'United Kingdom',
...
export async function fetchJurisdictions(): Promise<JurisdictionType[]> {
	await delay(500)
	...
	return Math.random() > 0.2 ? Promise.resolve(jurisdictions) : Promise.reject(new Error('Failed to fetch jurisdictions'))
}
...
function findJurisdiction(jurisdictionId: number): JurisdictionType | null {
	return findJurisdictionRecursive(fakeJurisdictionsData, jurisdictionId)
}

export async function fetchJurisdiction(jurisdictionId: number): Promise<JurisdictionType> {
	await delay(1000)
...
export async function fetchSubJurisdictions(jurisdictionId: number): Promise<JurisdictionType[]> {
	await delay(1500)
	const jurisdiction = findJurisdiction(jurisdictionId)
	...
	return Math.random() > 0.2 ? Promise.resolve((jurisdiction.subJurisdictions || []).map((j) => ({ id: j.id, name: j.name }))) : Promise.reject(new Error('Failed to fetch sub jurisdictions'))
}
